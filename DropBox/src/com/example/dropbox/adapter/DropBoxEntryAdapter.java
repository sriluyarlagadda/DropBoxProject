...
public class DropBoxEntryAdapter extends ArrayAdapter<Entry> {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (entry.isDir == false) {
			EntryFliginListener flingListener = new EntryFliginListener(
					listEntry);
		...
		if (entry.thumbExists) {
			...
			entryIcon.setImageResource(mContext.getResources().getIdentifier(
					"com.example.dropbox:drawable/" + entry.icon + "48", null,
					null));
		}
		...
		public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX,
			...
			if (view != null) {
				if (dx > 0) {
					...
					ObjectAnimator animeX = ObjectAnimator.ofFloat(
							view.findViewById(R.id.entryDisplay), "x",
							(float) view.getWidth());
					animeX.start();
					...
					View hiddenView = view.findViewById(R.id.hiddenView);
					ObjectAnimator animeX = ObjectAnimator.ofFloat(
							view.findViewById(R.id.entryDisplay), "x", 0);
